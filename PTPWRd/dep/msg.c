...
//void msgPackFollowUp(void *buf,Timestamp *preciseOriginTimestamp,PtpPortDS *ptpPortDS)
{
   ...
   //residence time for TCs
   put_be32(buf+8, ptpPortDS->frac_residence_time.nsec);
	put_be32(buf+12, ptpPortDS->frac_residence_time.phase);
...
/*Unpack Follow_up message from IN buffer of ptpPortDS to msgtmp.follow*/
void msgUnpackFollowUpResideceTime(void *buf, frac_resi_time *residenceTime)
{
   /*Residence Time*/
   residenceTime->nsec = get_be32(buf+8);
   residenceTime->phase = get_be32(buf+12);
}
...
void msgPackPDelayResp(void *buf,MsgHeader *header, PtpPortDS *ptpPortDS)
{
	/*changes in header*/
	*(char*)(buf+0)= *(char*)(buf+0) & 0xF0; //RAZ messageType
...
void msgPackPDelayRespFollowUp(void *buf,MsgHeader *header,PtpPortDS *ptpPortDS)
{
	/*changes in header*/
	*(char*)(buf+0)= *(char*)(buf+0) & 0xF0; //RAZ messageType
