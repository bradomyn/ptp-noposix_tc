...
int wr_servo_got_psync(PtpPortDS *clock, TimeInternal t1, TimeInternal t2)
{
	wr_servo_state_t *s = &clock->wr_servo;
   ...
   s->residence_time = clock->residence_time;
...
int wr_servo_got_pdelay(PtpPortDS *clock)
{
	wr_servo_state_t *s = &clock->wr_servo;

	s->t3 = clock->pDelayReq_tx_ts;
	//  s->t3.phase = 0;

	s->t4 = timeint_to_wr(clock->pdelay_req_receive_time);
	s->t4.correct = 1; //clock->delay_req_receive_time.correct;
	s->t4.phase = (int64_t) clock->PdelayReqHeader.correctionfield.lsb * 1000LL / 65536LL;

   s->t5 = timeint_to_wr(clock->pdelay_resp_send_time);
   s->t5.correct = 1;

   s->t6 = timeint_to_wr(clock->pdelay_resp_receive_time);
	s->t6.correct = 1; //clock->pdelay_resp_receive_time.correct;
	s->t6.phase = (int64_t) clock->PdelayRespHeader.correctionfield.lsb * 1000LL / 65536LL;

	return 0;
}


int wr_servo_update(PtpPortDS *clock, RunTimeOpts *rtOpts)
{
	wr_servo_state_t *s = &clock->wr_servo;

	uint64_t tics;

	wr_timestamp_t ts_offset_hw; /* ts_phase_adjust */
   ...
   else if(rtOpts->E2E_mode) // delay reques respond
   {
      delay_req_resp_offset(s, &ts_offset_hw);

   }else{ // peer to Peer
      //peer_delay_test(s, &ts_offset_hw);
      peer_delay_offset(s, &ts_offset_hw);
   }
...
int delay_req_resp_offset(wr_servo_state_t *s, wr_timestamp_t *ts_offset_hw)
{
   uint64_t big_delta_fix;
   uint64_t delay_ms_fix;
	wr_timestamp_t ts_offset;
...
int peer_delay_tc(wr_servo_state_t *s)
{
   uint64_t big_delta_fix;
   uint64_t delay_sm_fix;
   wr_timestamp_t ts_offset;

   printf("\n Servo : TimestampsIncorrect: %d %d %d %d\n",
            s->t3.correct, s->t4.correct, s->t5.correct, s->t6.correct);

   if(!s->t3.correct || !s->t4.correct || !s->t5.correct || !s->t6.correct)
   {
      PTPD_TRACE(TRACE_SERVO, NULL, "servo: TimestampsIncorrect: %d %d %d %d\n",
             s->t3.correct, s->t4.correct, s->t5.correct, s->t6.correct);
	...
	if (1) { /* enable for debugging */
		dump_timestamp("servo:t3", s->t3);
		dump_timestamp("servo:t4", s->t4);
		dump_timestamp("servo:t5", s->t5);
		dump_timestamp("servo:t6", s->t6);
   ...
   delay_sm_fix = (int64_t)((ts_to_picos(s->mu) - big_delta_fix) >> 1)
		+ s->delta_rx_m + s->delta_tx_s + ph_adjust;

   //delay_sm_fix = (((int64_t)(ts_to_picos(s->mu) - big_delta_fix) * (int64_t) s->fiber_fix_alpha) >> FIX_ALPHA_FRACBITS)
	//	+ ((ts_to_picos(s->mu) - big_delta_fix) >> 1)
	//	+ s->delta_tx_m + s->delta_rx_s + ph_adjust;

	ts_offset = ts_add(ts_sub(s->t5, s->t6), picos_to_ts(delay_sm_fix));
   dump_timestamp("servo:ts_offset_sm", ts_offset);
	...
	cur_servo_state.delay_ms = delay_sm_fix;
	cur_servo_state.total_asymmetry =
		(cur_servo_state.mu - 2LL * (int64_t)delay_sm_fix);
	cur_servo_state.fiber_asymmetry =
		cur_servo_state.total_asymmetry
		+ (s->delta_tx_m + s->delta_rx_s)
		- (s->delta_rx_m + s->delta_tx_s);
   ...
   s->delta_sm = delay_sm_fix;
   printf("\n*********************** Delay %lld ***********************\n", s->delta_sm);
   return 0;

}

int tc_accomulative(frac_resi_time *rt,wr_servo_state_t *s, frac_resi_time *acc_rt)
{
   wr_timestamp_t delay_ts;
   wr_timestamp_t temp;

   printf("\n ------------ Residence Time in this switch:  %d:%d \n",rt->nsec, rt->phase);
   printf("\n ------------ Residence Time Acc so far:  %d:%d \n",acc_rt->nsec, acc_rt->phase);
   // residence time
   rt->nsec += acc_rt->nsec;
   rt->phase += acc_rt->phase;
   printf("\n ------------ Residence Time Acc:  %d:%d \n",rt->nsec, rt->phase);

   delay_ts.sec = 0;
   delay_ts.nsec = rt->nsec;
   delay_ts.phase = rt->phase;

   // delay
   printf("\n*********************** Delay Previous %lld \n", s->delta_sm);
   temp = picos_to_ts(s->delta_sm);
   printf("*********************** Delay Previous TS %lld:%d:%d \n",temp.sec,temp.nsec,temp.phase);

   delay_ts = ts_add(delay_ts,picos_to_ts(s->delta_sm));

   // to do check the overflow of phase and nsec, it shouldn't happen...
   dump_timestamp("\n*********************** Delay + RT:",delay_ts);
   rt->nsec    = delay_ts.nsec;
   rt->phase   = delay_ts.phase;

   return 0;
}

int peer_delay_offset(wr_servo_state_t *s, wr_timestamp_t *ts_offset_hw)
{

   uint64_t big_delta_fix;
   uint64_t delay_sm_fix;
   wr_timestamp_t t1_t2_residence;
   wr_timestamp_t ts_offset;

   if(!s->t1.correct || !s->t2.correct || !s->t3.correct || !s->t4.correct || !s->t5.correct || !s->t6.correct)
   {
      PTPD_TRACE(TRACE_SERVO, NULL, "servo: TimestampsIncorrect: %d %d %d %d %d %d\n",
            s->t1.correct, s->t2.correct, s->t3.correct, s->t4.correct, s->t5.correct, s->t6.correct);
  ...
  printf("----------------- Accumlative Residence Time %d:%d \n", s->residence_time.nsec, s->residence_time.phase);
   ...
   t1_t2_residence = ts_add(ts_sub(s->t1, s->t2),s->residence_time);

	//ts_offset = ts_add(ts_sub(s->t1, s->t2), picos_to_ts(delay_sm_fix));
	ts_offset = ts_add(t1_t2_residence, picos_to_ts(delay_sm_fix));
   dump_timestamp("servo:ts_offset_sm", ts_offset);
	...
	*ts_offset_hw = ts_hardwarize(ts_offset, s->clock_period_ps);
	...
	s->delta_ms = delay_sm_fix;

   printf(" Delay_sm_fix %lld \n",s->delta_ms);
   return 0;

}

int peer_delay_test(wr_servo_state_t *s, wr_timestamp_t *ts_offset_hw)
{
   ...
   uint64_t delay_sm_fix;
   wr_timestamp_t ts_offset;
   wr_timestamp_t ts_offset_ms;
   wr_timestamp_t mu_ms;
   ...
   s->mu = ts_sub(ts_sub(s->t6, s->t3), ts_sub(s->t5, s->t4));
   mu_ms = ts_sub(ts_sub(s->t6, s->t3), ts_sub(s->t5, s->t4));
   ...
   printf(" Delay_sm_fix %lld Delay_sm_fix %lld \n"
      , delay_sm_fix
      , delay_sm_fix);

   delay_ms_fix = (((int64_t)(ts_to_picos(mu_ms) - big_delta_fix) * (int64_t) s->fiber_fix_alpha) >> FIX_ALPHA_FRACBITS)
		+ ((ts_to_picos(mu_ms) - big_delta_fix) >> 1)
		+ s->delta_tx_m + s->delta_rx_s + ph_adjust;

   printf(" Delay_ms_fix %lld Delay_ms_fix %lld \n"
      , delay_ms_fix
      , delay_ms_fix);

	dump_timestamp("t6-t5", ts_sub(s->t6, s->t5));
	dump_timestamp("t4-t3", ts_sub(s->t4, s->t3));
   dump_timestamp("t2-t1", ts_sub(s->t1, s->t2));

	ts_offset = ts_add(ts_sub(s->t5, s->t6), picos_to_ts(delay_sm_fix));
   dump_timestamp("servo:ts_offset_sm", ts_offset);

	ts_offset_ms = ts_add(ts_sub(s->t1, s->t2), picos_to_ts(delay_ms_fix));
   dump_timestamp("servo:ts_offset_ms", ts_offset_ms);
