...
void handlePDelayReq(MsgHeader*,Octet*,ssize_t,Boolean,RunTimeOpts*,PtpPortDS*);
void handlePDelayResp(MsgHeader*,Octet*,ssize_t,Boolean,RunTimeOpts*,PtpPortDS*);
...
void issuePDelayResp(MsgHeader*,RunTimeOpts*,PtpPortDS*);
void issuePDelayRespFollowUp(MsgHeader*,RunTimeOpts*,PtpPortDS*);
...
void handle(RunTimeOpts *rtOpts, PtpPortDS *ptpPortDS)
{
	...
	*/
	...
	case PDELAY_REQ:
		handlePDelayReq(&ptpPortDS->msgTmpHeader, ptpPortDS->msgIbuf, length, isFromSelf, rtOpts, ptpPortDS);
		break;

	case PDELAY_RESP:
		//PTPD_TRACE(TRACE_PROTO, ptpPortDS,"handle ..... PDELAY_RESP\n");
		handlePDelayResp(&ptpPortDS->msgTmpHeader, ptpPortDS->msgIbuf, length, isFromSelf, rtOpts, ptpPortDS);
		break;
...
void handleSync(MsgHeader *header, Octet *msgIbuf, ssize_t length, TimeInternal *time, Boolean isFromSelf, RunTimeOpts *rtOpts, PtpPortDS *ptpPortDS)
{
   ...
   PtpPortDS *master_port;    // the sync and followup arrive from a "Slave" port and send to the "Master" ports
   int max_port, slave_port,i;
	...
	switch(ptpPortDS->portState)
	{
		...
		case PTP_SLAVE:
            ...
            if(rtOpts->TC_mode)
            {
               // this is a fast test, DO BETTER
               slave_port  = ptpPortDS->portIdentity.portNumber;
               max_port    = rtOpts->portNumber;
               master_port = ptpPortDS-(slave_port-1); // first port

					PTPD_TRACE(TRACE_PROTO, ptpPortDS,"handle ..... SYNC    : TC\n");

               for(i=0; i < max_port; i++)
               {
                  if(master_port->linkUP && (master_port->portIdentity.portNumber != slave_port))
                     issueSync(rtOpts, master_port);
                  master_port++;
               }
               ...
               for(i=0; i <= max_port; i++)
               {
                  ...
                  if(master_port->linkUP && (master_port->portIdentity.portNumber != slave_port))
                  {
                     master_port->residence_time = ts_sub(master_port->synch_tx_ts,ptpPortDS->current_rx_ts);

                     printf("Sync RX %lld %d %d \n", ptpPortDS->current_rx_ts.sec, ptpPortDS->current_rx_ts.nsec,
                                                     ptpPortDS->current_rx_ts.correct);

                     printf("Sync TX %lld %d %d \n", master_port->synch_tx_ts.sec, master_port->synch_tx_ts.nsec,
                                                     master_port->synch_tx_ts.correct);

                     printf("#################### Residence Time Port[%d] = %lld:%d:%d ####################### \n",
                                                     i+1, master_port->residence_time.sec,
                                                     master_port->residence_time.nsec, master_port->residence_time.phase);

                     master_port->frac_residence_time.nsec  = (unsigned int) master_port->residence_time.sec * 1e9 + master_port->residence_time.nsec;
                     master_port->frac_residence_time.phase = (unsigned int) master_port->residence_time.phase;

                     printf("#################### Residence FRAC Time Port[%d] = %d:%d ####################### \n",
                                                     i+1, master_port->frac_residence_time.nsec,
                                                     master_port->frac_residence_time.phase);
		...
		if (msgIsFromCurrentParent(header, ptpPortDS))
		{
			if (ptpPortDS->waitingForFollow)
			{
               ...
               if(rtOpts->TC_mode)
               {
                  ...
                  max_port    = rtOpts->portNumber;
                  master_port = ptpPortDS-(slave_port-1); // first port
                  // residence time from the follow up
                  msgUnpackFollowUpResideceTime(ptpPortDS->msgIbuf, &residenceTime);
                  ...
                  for(i=0; i <= max_port; i++)
                  {
                     ...
                     if(master_port->linkUP && (master_port->portIdentity.portNumber != slave_port))
                     {
                        // the sync and follow up msg arrive from the "slave port"
                        //master_port->frac_residence_time.nsec = master_port->frac_residence_time.nsec + residenceTime.nsec;
                        //master_port->frac_residence_time.phase= master_port->frac_residence_time.phase + residenceTime.phase;
                        tc_accomulative(&(master_port->frac_residence_time), &(ptpPortDS->wr_servo), &residenceTime);

                        //copy the t1 ts
                        master_port->synch_tx_ts.sec = (int64_t) ptpPortDS->msgTmp.follow.preciseOriginTimestamp.secondsField.msb << 32
                                                       | ptpPortDS->msgTmp.follow.preciseOriginTimestamp.secondsField.lsb;
                        master_port->synch_tx_ts.nsec = ptpPortDS->msgTmp.follow.preciseOriginTimestamp.nanosecondsField;

		   		         issueFollowup(rtOpts,master_port);
                     }
                  }
                  break;
               }
               ...
               if (!rtOpts->E2E_mode)
		         {
                  ...
                  ptpPortDS->residence_time.sec    = 0;
                  ptpPortDS->residence_time.nsec   = residenceTime.nsec;
                  ptpPortDS->residence_time.phase  = residenceTime.phase;

                  printf("Residence Time RX %d:%d \n", ptpPortDS->residence_time.nsec, ptpPortDS->residence_time.phase);
                  wr_servo_got_psync(ptpPortDS, preciseOriginTimestamp, ptpPortDS->sync_receive_time);
	...
	case PTP_SLAVE:
		    ...
		    && isFromCurrentParent)
		{
   		...
   		ptpPortDS->delay_req_receive_time.nanoseconds = requestReceiptTimestamp.nanoseconds;
			...
			wr_servo_update(ptpPortDS, rtOpts);
...
void handlePDelayReq(MsgHeader *header, Octet *msgIbuf, ssize_t length, Boolean isFromSelf, RunTimeOpts *rtOpts, PtpPortDS *ptpPortDS)
{
  ...
  if (!rtOpts->E2E_mode)
   {
	...
	switch(ptpPortDS->portState )
	{
		...
		{
			PTPD_TRACE(TRACE_PROTO, ptpPortDS,"handle  ..... PDELAY_REQ:, succedded\n");
			msgUnpackHeader(ptpPortDS->msgIbuf,&ptpPortDS->PdelayReqHeader);

         /* FIXME: do this, but properly */
         ptpPortDS->PdelayReqHeader.correctionfield.msb = 0;
         ptpPortDS->PdelayReqHeader.correctionfield.lsb = phase_to_cf_units(ptpPortDS->current_rx_ts.phase);

			//issuePDelayResp(header,rtOpts,ptpPortDS);
			issuePDelayResp(&ptpPortDS->PdelayReqHeader,rtOpts,ptpPortDS);
		   issuePDelayRespFollowUp(&ptpPortDS->PdelayReqHeader,rtOpts,ptpPortDS);
...
void handlePDelayResp(MsgHeader *header, Octet *msgIbuf,ssize_t length, Boolean isFromSelf, RunTimeOpts *rtOpts, PtpPortDS *ptpPortDS)
{
	...
	switch(ptpPortDS->portState )
	{
		...
		case PTP_SLAVE:
		   ...
		   if (isFromSelf)
		   {
			...
			PTPD_TRACE(TRACE_PROTO, ptpPortDS,"handle ..... PDELAY_RESP:, succedded\n");

         // t6
			ptpPortDS->pdelay_resp_receive_time.seconds      = ptpPortDS->current_rx_ts.sec;
			ptpPortDS->pdelay_resp_receive_time.nanoseconds  = ptpPortDS->current_rx_ts.nsec;
			ptpPortDS->pdelay_resp_receive_time.phase        = ptpPortDS->current_rx_ts.phase;
			ptpPortDS->pdelay_resp_receive_time.correct      = ptpPortDS->current_rx_ts.correct;

         ptpPortDS->PdelayRespHeader.correctionfield.msb = 0;
         ptpPortDS->PdelayRespHeader.correctionfield.lsb = phase_to_cf_units(ptpPortDS->current_rx_ts.phase);


         // t4
         /* copy timestamp */
         toInternalTime(&ptpPortDS->pdelay_req_receive_time, &ptpPortDS->msgTmp.presp.requestReceiptTimestamp);
         /*copy correction time*/
         ptpPortDS->PdelayReqHeader.correctionfield.msb = header->correctionfield.msb;
         ptpPortDS->PdelayReqHeader.correctionfield.lsb = header->correctionfield.lsb;
			//integer64_to_internalTime(header->correctionfield,&ptpPortDS->lastPdelayReqCorrectionField);
...
if (!rtOpts->E2E_mode)
 {
	...
	switch(ptpPortDS->portState)
	{
		...
		if (header->sequenceId == ptpPortDS->sentPDelayReqSequenceId-1)
		{
         ...
         PTPD_TRACE(TRACE_PROTO, ptpPortDS,"handle ..... PDELAY_RESP_FOLLOW_UP:, succedded\n");

			toInternalTime(&ptpPortDS->pdelay_resp_send_time,&ptpPortDS->msgTmp.prespfollow.responseOriginTimestamp);

		   //integer64_to_internalTime(ptpPortDS->msgTmpHeader.correctionfield,&correctionField);
			//addTime(&correctionField,&correctionField,&ptpPortDS->lastPdelayRespCorrectionField);

         wr_servo_got_pdelay(ptpPortDS);

         //tc
         if(rtOpts->TC_mode)
         {
            peer_delay_tc(&ptpPortDS->wr_servo);
            break;
         }
...
void issuePDelayReq(RunTimeOpts *rtOpts,PtpPortDS *ptpPortDS)
{
	...
	{
		PTPD_TRACE(TRACE_PROTO, ptpPortDS,"issue  ..... PDELAY_REQ: succedded [timestamp: %s]\n",format_wr_timestamp(ptpPortDS->delayReq_tx_ts));
		ptpPortDS->sentPDelayReqSequenceId++;
	}
}

/*Pack and send on event multicast ip adress a PDelayResp message*/
void issuePDelayResp(MsgHeader *header,RunTimeOpts *rtOpts,PtpPortDS *ptpPortDS)
{
	msgPackPDelayResp(ptpPortDS->msgObuf,header,ptpPortDS);
...
void issuePDelayRespFollowUp(MsgHeader *header,RunTimeOpts *rtOpts,PtpPortDS *ptpPortDS)
{

	//msgPackPDelayRespFollowUp(ptpPortDS->msgObuf,header,ptpPortDS->pDelayResp_tx_ts,ptpPortDS);
	msgPackPDelayRespFollowUp(ptpPortDS->msgObuf,header,ptpPortDS);
